            ...
            <div class="modal-body">
                ...
                <div class="row mb-3">
                    <div class="col-md">
                        ...
                        <input id="adviserInputAmount" class="form-control form-control-md mb-2" type="text" name="adviser_amount" placeholder="Enter Honorarium Amount" disabled/>
                    </div>
                    <div class="col-md">
                        <input type="hidden" id="chairperson_id" name="chairperson_id">
                        <div>Chairperson: &nbsp;<b class="text-primary" id="chairperson"></b></div>
                        <input id="chairpersonInputAmount" class="form-control form-control-md mb-2" type="text" name="chairperson_amount" placeholder="Enter Honorarium Amount" disabled/>
                    </div>
                </div>
                <div class="row mb-3">
                    <div class="col-md">
                ...
                <div class="row mb-2">
                    <div class="col-md">
                        ...
                        <input id="recorderInputAmount" class="form-control form-control-md mb-2" type="text" name="recorder_amount" placeholder="Enter Honorarium Amount" disabled/>
                    </div>
                </div>
                <div class="row">
                    <div class="col-md d-flex justify-content-end gap-2">
                        @if(in_array(Auth::user()->usertype->name, ['Superadmin', 'Administrator']))
                            <button type="button" class="btn btn-label-danger" data-bs-dismiss="modal">Close</button>
                            <button type="button" class="btn btn-success" id="editAmount">Edit</button>
                            <button type="submit" class="btn btn-primary" id="saveAmount" style="display: none;">Save Amount</button>
                        @endif
                    </div>
                </div>
            </div>
...
<script>
    $(function () {
        ...
        var thesisOpenOutgoingTable = $('#thesisOpenOutGoingTable').DataTable({
            ...
            columns: [
                ...
                { data: 'recorder', name: 'recorder', title: 'Recorder', render: function(data, type, row) {
                    return `<div class="text-primary text-uppercase">${data}</div>`;
                } },
                { data: 'date', name: 'date', title: 'Date' },
                @if(Auth::user()->usertype->name === 'Superadmin' || 'Administrator' || 'Budget Office' || 'Dean' || 'Accounting' || 'Cashiers')
                {
                    data: 'action',
                    name: 'action',
                    title: 'Action',
                    ...
                    render: function(data, type, row) {
                        return `
                            <div class="d-flex">
                                <button type="button" class="btn btn-sm btn-primary add-btn" data-id="${row.id}" data-bs-toggle="modal" data-bs-target="#addHonorariumAmount">Amount</button>
                            </div>
                        `;
                    }
                }
    ...
    $(document).on('click', '.add-btn', function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                // Format defense date as "Month day, Year"
                if (data.transaction.defense_date) {
                    let formattedDate = new Date(data.transaction.defense_date).toLocaleDateString('en-US', {
                        month: 'long', day: 'numeric', year: 'numeric'
                    });
                    $('#defense_date').text(formattedDate);
                } else {
                    $('#defense_date').text('N/A');
                }
                // $('#defense_time').text(data.transaction.defense_time || 'N/A');
                // Format defense time to "hh:mm AM/PM"
                if (data.transaction.defense_time) {
                    let timeString = data.transaction.defense_time;
                    let [hours, minutes] = timeString.split(':').map(Number);
                    let period = hours >= 12 ? 'PM' : 'AM';
                    hours = hours % 12 || 12; // Convert 0 to 12 for 12AM
                    let formattedTime = `${hours}:${String(minutes).padStart(2, '0')} ${period}`;
                    $('#defense_time').text(formattedTime);
                } else {
                    $('#defense_time').text('N/A');
                }
                ...
                data.members.forEach(member => {
                    ...
                    membersHtml += `
                        ...
                        <input class="form-control form-control-md mb-2 member-input" id="membersListField" type="text" name="member_ids[${member.id}]" placeholder="Enter Honorarium Amount" value="${memberAmount}" disabled/>
                    `;
                });
    ...
    $(document).ready(function () {
        ...
        $("#editAmount").click(function () {
            // Enable input fields
            $("#adviserInputAmount, #chairpersonInputAmount, #recorderInputAmount, #membersListField").prop("disabled", false);
        ...
        // When modal is closed, reset fields to disabled
        $("#addHonorariumAmount").on("hidden.bs.modal", function () {
            $("#adviserInputAmount, #chairpersonInputAmount, #recorderInputAmount, #membersListField").prop("disabled", true);
