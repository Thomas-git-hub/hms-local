...
class AdminController extends Controller
{
    ...
    public function admin_on_hold(Request $request){
        if(Auth::user()->usertype->name === 'Faculty'){
            ...
            return view('administration.admin_on_hold', compact('OnHold', 'EmailCount', 'OnHoldData', 'office', 'hasForCompliance'));
    ...
    public function getAcknowledgement(){
        $acknowledgementCount= Acknowledgement::with(['transaction'])
        ->where('office_id', Auth::user()->office_id)
        ->whereHas('transaction', function($query) {
            $query->where('status', 'On Queue'); // Added condition to filter by transaction status
        })
        ->count();
