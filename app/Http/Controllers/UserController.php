...
class UserController extends Controller
{
    ...
    public function getUser(Request $request) {
        ...
        $faculties = DB::connection('ors_pgsql')
                ...
                $query->where(DB::raw("LOWER(CONCAT(employee.employee_fname, ' ', employee.employee_lname))"), 'like', "%".strtolower($searchTerm)."%")
                    ->orWhere(DB::raw("LOWER(employee.employee_fname)"), 'like', "%".strtolower($searchTerm)."%")
                    ->orWhere(DB::raw("LOWER(employee.employee_lname)"), 'like', "%".strtolower($searchTerm)."%");
                })
                ->select(
